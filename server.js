...
// チャットメッセージ送信
app.post('/api/chat/send', chatLimiter, authenticateToken, async (req, res) => {
    try {
...
// チャットメッセージ取得API
app.get('/api/chat/:partyId', authenticateToken, async (req, res) => {
    try {
...
app.post('/api/auth/register', authLimiter, async (req, res) => {
    try {
        const { username, displayName, summonerName, password } = req.body;

        // 入力検証
        if (!username || !displayName || !summonerName || !password) {
            return res.status(400).json({ 
                message: '全ての項目を入力してください',
                details: { username, displayName, summonerName }
            });
        }
        ...
        const user = await supabase.createUser(userData);

        // JWTトークンを生成
        const token = jwt.sign(
            { id: user.id, username: user.username },
            process.env.JWT_SECRET,
            { expiresIn: '30d' } // 30日に延長
        );

        // HTTPOnlyクッキーにトークンを設定（有効期限も延長）
...
// サモナー情報更新API
app.post('/api/user/refresh-summoner', authenticateToken, async (req, res) => {
    try {
        const userId = req.user.id;
...
// 特定のパーティー情報取得API
app.get('/api/parties/:partyId', authenticateToken, (req, res) => {
    try {
