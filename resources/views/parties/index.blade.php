...
<head>
    ...
    <title>パーティー一覧 - League of Legends フレンド募集</title>
</head>
<body>
    <header class="header">
        <nav class="nav">
    ...
    <div class="parties-container">
        <div class="parties-header">
            <h1 class="parties-title">パーティー一覧</h1>
        </div>
            ...
            @forelse($parties as $party)
                <div class="party-card">
                    ...
                    <div class="party-members">
                        <h4>メンバー ({{ $party->members->count() }}人)</h4>
                    ...
                    <div class="party-actions">
                        @if($party->status === 'active')
                            <a href="{{ route('parties.show', $party) }}" class="enter-btn">
                                <span class="enter-icon">🚪</span>
                                パーティールームに入る
                            </a>
                            @if($party->isLeader(Auth::id()))
                                <button type="button" class="close-btn" onclick="showClosePartyDialog({{ $party->id }}, '{{ $party->name }}')">
                                    <span class="close-icon">🔒</span>
                                    パーティーを閉じる
                                </button>
                            @endif
                        @else
                            <span class="closed-badge">終了済み</span>
                        @endif
                    </div>
                </div>
            @empty
                <div class="no-parties">
                    <p>まだ参加しているパーティーがありません。</p>
                    <a href="{{ route('recruitment.index') }}" class="find-party-btn">募集を探す</a>
                </div>
            @endforelse
        </div>
    </div>
...
.member-avatar {
    ...
    overflow: hidden;
    border-radius: 50%;
    background-color: #00bfff; /* Placeholder background */
}
...
<div id="close-party-modal" class="custom-modal">
    <div class="custom-modal-content">
        ...
        <div class="custom-modal-body">
            <p>以下のパーティーを閉じますか？</p>
            <p class="party-title" id="close-party-title"></p>
            <p class="warning-text">この操作は取り消すことができません。</p>
        </div>
        <div class="custom-modal-footer">
            <form id="close-party-form" method="POST" style="display: inline;">
                @csrf
                @method('PATCH')
...
<script>
    ...
    // パーティーを閉じるダイアログの表示
    function showClosePartyDialog(partyId, partyName) {
        document.getElementById('close-party-title').textContent = partyName;
        document.getElementById('close-party-form').action = `/parties/${partyId}/close`;
